 ...
 */
export function getVisibleNavigation(userPermissions: string[], t: TFunction) {
  const nav: any[] = [];

  const showUsers = hasUiAccess(userPermissions, "users");
  const showRoles = hasUiAccess(userPermissions, "roles");
  ...
  // Research — visible to every authenticated user (read-only survey bank).
