...
const MissionConceptsPage: React.FC = () => {
  ...
  return <Box data-language-switcher sx={{ minHeight: "100dvh", color: INK, bgcolor: "#f4f0fa", backgroundImage: "radial-gradient(circle at 12% 55%,rgba(136,94,193,.08),transparent 32%),radial-gradient(circle at 88% 70%,rgba(136,94,193,.07),transparent 30%)" }}>
    ...
    <Container maxWidth="lg" sx={{ mt: { xs: -3, md: -4 }, pb: 2, position: "relative", zIndex: 2 }}>
      ...
      <Box sx={{ textAlign: "center", py: { xs: 2, md: 3 } }}><Typography component="h2" sx={{ fontFamily: SERIF, fontSize: { xs: 38, md: 52 }, lineHeight: 1.1 }}>{copy(lang, "Key concepts", "Conceptos clave")}</Typography><Typography sx={{ color: PURPLE, fontSize: { xs: 16, md: 18 }, mt: 1.5 }}>{copy(lang, "Vulnerability · Vicarious trauma · “What now?”", "Vulnerabilidad · Trauma vicario · “¿Y ahora qué?”")}</Typography><Box sx={{ width: 34, borderTop: `1px solid ${PURPLE}`, mx: "auto", my: 2 }} /><Typography sx={{ maxWidth: 660, mx: "auto", lineHeight: 1.55 }}>{copy(lang, "Understanding key concepts helps us make sense of what we feel, validate our responses, and open space for connection and healing.", "Comprender los conceptos clave nos ayuda a darle sentido a lo que sentimos, validar nuestras respuestas y abrir espacio a la conexión y la sanación.")}</Typography></Box>
