...
export function _seedCacheForTesting(
  entries: Record<string, string[] | { permissions: string[]; bypassFeatureChecks: boolean }>
): void {
  _cache.clear();
  for (const [role, value] of Object.entries(entries)) {
    if (Array.isArray(value)) {
      _cache.set(role, { permissions: new Set(value), bypassFeatureChecks: false });
    } else {
      _cache.set(role, { permissions: new Set(value.permissions), bypassFeatureChecks: value.bypassFeatureChecks });
    }
  }
}
